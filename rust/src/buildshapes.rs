...
use crate::vec3::Vec3;
//use crate::vec2::Vec2;
use crate::geometry::{Shape,Face,Edge};
...
pub fn build_cylinder(r : Field, h : Field, n : VertIndex) -> Shape<Vec3> {
	...
	let cap_coords : Vec<Vec3> = angles.map(|angle| Vec3(angle.cos(),angle.sin(),0.0)*r).collect();
	//let cap_coords= angles.map(|angle| Vec3(angle.cos(),angle.sin(),0.0)*r);
	let n_angles = (0..n).map(|i| 2.0*PI*((i as Field)+0.5)/(n as Field));
	let normals = n_angles.map(|angle| Vec3(angle.cos(),angle.sin(),0.0)*r);
	//build verts
	let top_verts = cap_coords.iter().map(|v| *v + Vec3(0.0,0.0,h/2.0));
	let bottom_verts = cap_coords.iter().map(|v| *v + Vec3(0.0,0.0,-h/2.0));
