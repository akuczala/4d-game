...
use crate::draw::visual_aids::{calc_grid_lines, draw_stars, draw_horizon, draw_sky};
...
use crate::constants::{PI, CUBE_LABEL_STR, COIN_LABEL_STR, N_FUZZ_LINES};
...
pub fn build_corridor_cross<V : VectorTrait>(cube_builder: &ShapeEntityBuilderV<V>, wall_length : Field) -> Vec<ShapeEntityBuilderV<V>> {
    ...
    pub fn build_texture<V : VectorTrait>(shape : &Shape<V>, scale: &Scaling<V>) -> ShapeTexture<V::SubV> {
        ...
        for (face, face_texture) in shape.faces.iter().zip(cube_texture.face_textures.iter_mut()) {
            ...
            .merged_with(&draw::Texture::make_fuzz_texture(N_FUZZ_LINES).set_color(target_face_color))
    ...
    V::M: Componentable
{
    ...
    for (axis,dir) in iproduct!(match V::DIM {3 => vec![0,2], 4 => vec![0,2,3], _ => panic!("Invalid dimension")},vec![-1.,1.]) {
        insert_coin(
            world,
            ShapeEntityBuilder::new_convex_from_ref_shape(&ref_shapes, ShapeLabel::from_str(COIN_LABEL_STR))
