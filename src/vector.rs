...
pub fn linspace(min: Field, max: Field, n: usize) -> impl Iterator<Item = Field> {
    (0..n)
        .map(move |i| (i as Field) / ((n - 1) as Field))
        .map(move |f| (1. - f) * min + f * max)
}
...
// TODO: check if we really need all these bounds
// at one time tried + Mul<Field,Output=Self> + Mul<Self,Output=Self>
//weirdly only the last Mul is remembered
pub trait MatrixTrait<V: VectorTrait>:
    Display
    + Copy
    + Add<Output = Self>
    + Sub<Output = Self>
...
pub trait Rotatable<V: VectorTrait> {
    ...
    fn rotate(&mut self, axis1: VecIndex, axis2: VecIndex, angle: Field) {
        let rot_mat = rotation_matrix(
            self.get_frame()[axis1],
            self.get_frame()[axis2],
            Some(angle),
        );
    ...
    V: VectorTrait,
{
    ...
    println!(
        "correct rotation: {}",
        V::is_close(v2.normalize(), mat * v1.normalize())
    );
...
fn test_barycenter() {
    ...
    assert!(
        ...
        "center_from_iter={}, expected_center={}",
        center_from_iter,
        expected_center
    );
}
