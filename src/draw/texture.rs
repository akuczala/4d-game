use super::{DrawLine,LineGenerator,DrawLineGenerator};
use super::{Buffer};
...
//this would likely work fine
//but it seems awkward to have the texture itself be the generator
//even more so for faces, shapes etc
//maybe not for faces, since it is TextureMapping which generates the lines
//we need to also feed initial data to texture mapping before it starts iterating
//so we can't just call next() out of the blue
//
//the main conflict:
//we want an iterator struct nestled within whatever object that returns lines
//..but this struct needs access to its "parent's" data to return these lines
//maybe the struct could hold a bunch of refs to parent data that it gets
//when it is created?
//do we have a different struct for each kind of object, or can we 
//somehow take a closure as an argument that tells it what to do?
//worth reading up on rust closures /applications again

impl<V: VectorTrait> DrawLineGenerator<V> for Texture<V> {}
impl<V : VectorTrait> Iterator for Texture<V> {
	type Item = DrawLine<V>;
...
impl TextureMapping {
	pub fn draw<V : VectorTrait>(&self, buffer : &mut Buffer<Option<DrawLine<V>>>, face : &Face<V>, shape : &Shape<V>, face_scales : &Vec<Field>
	) {
		if !face.visible && !shape.transparent  {
			return
		}
		match &face.texture {
			Texture::DefaultLines{color} => self.draw_default_lines(buffer,face,shape,*color,face_scales),
			Texture::Lines{lines,color} => self.draw_lines(buffer,shape,lines,*color),
			Texture::DrawLines(draw_lines) => self.draw_drawlines(buffer,draw_lines)

		}
		
	}
	pub fn draw_default_lines<V : VectorTrait>(
		&self, buffer: &mut Buffer<Option<DrawLine<V>>>, face : &Face<V>, shape : &Shape<V>,
		color : Color, face_scales : &Vec<Field>) {
	...
	pub fn draw_lines<V : VectorTrait>(&self, buffer : &mut Buffer<Option<DrawLine<V>>>, shape : &Shape<V>,
		lines : &Vec<Line<V::SubV>>, color : Color) {
		...
		let line_iter = lines.iter().map(|line|
	...
	pub fn draw_default_lines_returnvec<V : VectorTrait>(
	...
	pub fn draw_lines_returnvec<V : VectorTrait>(&self, shape : &Shape<V>,
	...
	// 	lines : &Vec<Line<V::SubV>>, color : Color) -> Vec<Option<DrawLine<V>>> {
	// 	self.draw_lines_iter(shape,lines,color).collect()
	// }
	// pub fn draw_lines_iter<'a,V : VectorTrait + 'a>(&self, shape : &Shape<V>,
	// 	lines : &'a Vec<Line<V::SubV>>, color : Color) -> impl Iterator<Item=Option<DrawLine<V>>> +'a {
	// 	let origin = shape.verts[self.origin_verti];
	// 	let frame_verts : Vec<V> = self.frame_vertis.iter().map(|&vi| shape.verts[vi] - origin).collect();
	// 	//this is pretty ridiculous. it just matrix multiplies a matrix of frame_verts (as columns) by each vertex
	// 	//in every line, then adds the origin.
	// 	lines.iter().map(move |line|
	// 		line.map(move |v| (0..V::SubV::DIM).zip(frame_verts.iter()).map(|(i,&f)| f*v[i]).fold(V::zero(),|a,b| a + b) + origin)
	// 		).map(move |line| Some(DrawLine{line,color}))
	// }

	//does nothing atm
	pub fn draw_drawlines<V : VectorTrait>(&self, buffer : &mut Buffer<Option<DrawLine<V>>>,
		draw_lines : &Vec<DrawLine<V::SubV>>) {
