        ...
        if keys[pygame.K_RALT] or keys[pygame.K_LALT]:
            for key_id in trans_keymapping:
                if keys[key_id]:
                    ...
                    self.pos = self.pos + trans_sign * self.speed * self.frame[
                        axis]
                    update = True
        else:
            for key_id in rot_keymapping:
                if keys[key_id]:
                    ...
                    self.update_rot_matrix(axis1, axis2,
                                           angle_sign * self.ang_speed)
                    update = True
        #mouse
        ...
        if self.enable_mouse:
            ...
            for event in events:
                if event.type == pygame.MOUSEMOTION:
                    ...
                    mx, my = event.pos
                    dmx, dmy = mx - draw_class.center[
                        0], my - draw_class.center[1]
            ...
            if abs(dmx) > 2 or abs(dmy) > 2:
                ...
                if d == 3:
                    self.update_rot_matrix(
                        1, 2, -dmy / draw_class.height * 32 * self.ang_speed)
                    self.update_rot_matrix(
                        0, 2, dmx / draw_class.height * 32 * self.ang_speed)
                if d == 4:
                    if keys[pygame.K_RSHIFT] or keys[pygame.K_LSHIFT]:
                        self.update_rot_matrix(
                            2, 3,
                            dmx / draw_class.height * 32 * self.ang_speed)
                    else:
                        self.update_rot_matrix(
                            0, 3,
                            dmx / draw_class.height * 32 * self.ang_speed)
                    self.update_rot_matrix(
                        1, 3, -dmy / draw_class.height * 32 * self.ang_speed)
                mouse_in = True
